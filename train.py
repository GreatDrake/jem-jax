...
parser.add_argument("--reinit_freq", type=float, default=.05) # probability p of initialising starting point for sgld from Uniform[-1;1], with probability 1 - p the point is taken from replay buffer
...
parser.add_argument("--p_x_weight", type=float, default=1.) # weight of loss -LogSumExp(f(x)) + LogSumExp(f(x_hat))
args = parser.parse_args()


print("data init started")
data_source = input_pipeline.CIFAR10DataSource(
        train_batch_size=args.batch_size, eval_batch_size=10000)
